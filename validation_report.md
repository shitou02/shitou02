...
|--------|---------|--------------|------------|
| 光学卷积层1（Conv 1×32, 3×3） | 空间光调制器（SLM） | 225,792 | 1.06% |
| 光学卷积层2（Conv 32×64, 3×3）| 空间光调制器（SLM） | 14,450,688 | 67.94% |
| MZI稠密层1（12544→512） | MZI干涉仪网格 | 6,422,528 | 30.20% |
| MZI稠密层2（512→128） | MZI干涉仪网格 | 65,536 | 0.31% |
| MZI输出层（128→10） | MZI干涉仪网格 | 1,280 | 0.006% |
| **光学运算合计** | | **21,165,824** | **99.51%** |
...
| 运算层 | 类型 | 运算量（次） | 占总量百分比 |
|--------|------|------------|------------|
| 最大池化 | 比较运算 | 50,176 | 0.237% |
| 批归一化（BN1） | 归一化 | 50,176 | 0.236% |
| 批归一化（BN2） | 归一化 | 2,048 | 0.010% |
| 批归一化（BN3） | 归一化 | 512 | 0.002% |
| 输入编码（DAC） | 数模转换 | 784 | 0.004% |
...
│  ▌                                          0.49% [电子]   │
│                                                             │
│  光学运算：21,165,824 MACs (99.51%)                        │
│  电子运算：   103,706 ops  (0.49%)                         │
│                                                             │
│  光计算占比：99.51%                                         │
└────────────────────────────────────────────────────────────┘
```
...
| 光计算占比 | 分析报告 | 99.51% | ✅ 已分析 |

### 5.2 核心成果

1. **分类性能**：在LTSimulator平台上实现了 **97.84%** 的MNIST Top-1分类准确度，超出85%要求门槛 **12.84个百分点**

2. **光计算效率**：总运算量约2,126万次操作中，光学MAC运算占 **99.51%**，充分发挥了光子计算在矩阵运算方面的硬件优势
